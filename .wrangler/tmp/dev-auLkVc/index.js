...
// .wrangler/tmp/bundle-qI228m/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-qI228m/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
async function loadConfig(env) {
  try {
    ...
    console.error("Error loading config from D1:", error3);
  }
...
async function generateQRCode(data) {
  try {
    ...
    console.error("QR code generation failed:", error3);
    return "data:image/png;base64,placeholder";
  }
}
__name(generateQRCode, "generateQRCode");
async function sendTicketEmail(env, email, tickets, eventInfo) {
  try {
    const ticketsHtml = tickets.map((ticket) => `
      <div style="border: 2px solid #4ecdc4; border-radius: 15px; padding: 20px; margin: 20px 0; background: #f9f9f9;">
        ...
        <p style="color: #666;"><strong>Event:</strong> ${eventInfo.name}</p>
        <p style="color: #666;"><strong>When:</strong> ${eventInfo.day}, ${eventInfo.date} at ${eventInfo.time}</p>
        <p style="color: #666;"><strong>Where:</strong> ${eventInfo.address}</p>
        <div style="text-align: center; margin: 20px 0;">
          <img src="${ticket.qrCodeDataUrl}" alt="QR Code" style="max-width: 300px;"/>
          <p style="color: #888; font-size: 14px;">Show this QR code at the event entrance</p>
        </div>
      </div>
    `).join("");
    const emailHtml = `
      ...
      <html>
        ...
        <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="text-align: center; margin-bottom: 30px;">
            <h1 style="color: #4ecdc4; margin-bottom: 10px;">\u{1F39F}\uFE0F Your Tickets Are Ready!</h1>
          ...
          <div style="background: #f0f0f0; border-radius: 10px; padding: 20px; margin-top: 30px;">
            <p style="color: #666; margin: 0;"><strong>Need help?</strong></p>
            <p style="color: #666; margin: 5px 0 0 0;">
              You can view your tickets anytime at: 
              <a href="https://yoursite.com/confirmation?email=${encodeURIComponent(email)}" style="color: #4ecdc4;">
                View Tickets Online
              </a>
            </p>
          </div>
    ...
    console.error("Email sending failed:", error3);
    return false;
  }
}
...
async function generateAndSendTickets(env, email, paymentIntentId, quantity = 1) {
  ...
  await sendTicketEmail(env, email, generatedTickets, eventInfo);
...
app.post("/api/dev/generate-tickets", async (c) => {
  try {
    const { email, quantity = 1 } = await c.req.json();
    if (!email) {
      return c.json({ error: "Email is required" }, 400);
    }
    const testPaymentIntentId = "pi_test_" + Date.now();
    await generateAndSendTickets(c.env, email, testPaymentIntentId, quantity);
    return c.json({
      success: true,
      message: `Generated ${quantity} ticket(s) for ${email}`,
      confirmationUrl: `/confirmation?email=${encodeURIComponent(email)}`
    });
  } catch (error3) {
    console.error("Test ticket generation error:", error3);
    return c.json({ error: error3.message }, 500);
  }
});
...
app.post("/api/config", async (c) => {
  try {
    ...
    console.error("Error saving config:", error3);
    return c.json({ error: error3.message }, 500);
  }
});
app.get("*", async (c) => {
